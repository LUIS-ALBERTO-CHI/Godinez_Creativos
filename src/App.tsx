...
const WHATSAPP_NUMBER = '529995538401'; // 52 (MX) + 10 dígitos
const WHATSAPP_URL = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hola Godínez Creativos, me gustaría más información sobre sus servicios.')}`;
...
function App() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-black">
      ...
      <div className={`mobile-menu-overlay ${isMobileMenuOpen ? 'menu-active' : ''}`}>
        ...
        <div className="absolute bottom-6 left-0 w-full px-6 flex flex-col items-center gap-3 z-10">
          <div className="flex gap-4">
            <a href={FACEBOOK_URL} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full glass-panel flex items-center justify-center text-white/50 hover:text-[#f60566] hover:border-[#f60566]/40 transition-all active:scale-90" aria-label="Facebook">
      ...
      {/* Footer */}
      <footer className="relative z-10 border-t border-white/10 bg-black/60 backdrop-blur-sm px-6 md:px-16 pt-16 pb-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-white/60 font-light leading-relaxed max-w-xs">
              Marcas que se ven diferente, venden diferente. Branding, redes sociales, diseño y desarrollo web.
            </p>
          </div>

          {/* Navegación */}
          <div className="space-y-4">
            <h4 className="text-[11px] font-mono tracking-widest text-white/40 uppercase">Navegación</h4>
            <ul className="space-y-2.5">
              {[
                { label: 'Inicio', href: '#inicio' },
                { label: '¿Qué hacemos?', href: '#servicios' },
                { label: '¿Por qué nosotros?', href: '#diferencia' },
                { label: 'Portafolio', href: '#portafolio' },
              ].map((l) => (
                <li key={l.href}>
                  <a href={l.href} className="text-sm text-white/70 hover:text-[#f60566] transition-colors">{l.label}</a>
                </li>
              ))}
              <li>
                <button onClick={() => openJoinModal()} className="text-sm text-white/70 hover:text-[#f60566] transition-colors">Contacto</button>
              </li>
            </ul>
          </div>
          ...
          {/* Contacto */}
          <div className="space-y-4">
            <h4 className="text-[11px] font-mono tracking-widest text-white/40 uppercase">Hablemos</h4>
            <div className="space-y-2.5">
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-sm text-white/70 hover:text-[#f60566] transition-colors">
              ...
              <a href={`mailto:${CONTACT_EMAIL}`} className="flex items-center gap-2.5 text-sm text-white/70 hover:text-[#f60566] transition-colors break-all">
            ...
            <div className="flex gap-3 pt-1">
              <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-9 h-9 rounded-full border border-white/15 flex items-center justify-center text-white/60 hover:text-[#f60566] hover:border-[#f60566]/40 transition-all active:scale-90">
              ...
              <a href={FACEBOOK_URL} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-9 h-9 rounded-full border border-white/15 flex items-center justify-center text-white/60 hover:text-[#f60566] hover:border-[#f60566]/40 transition-all active:scale-90">
      ...
      <a
        ...
        className="fixed bottom-5 right-5 z-40 flex items-center justify-center w-14 h-14 rounded-full bg-[#25D366] text-white shadow-[0_8px_30px_rgba(37,211,102,0.45)] hover:scale-110 active:scale-95 transition-transform duration-300"
      >
        <span className="absolute inline-flex h-full w-full rounded-full bg-[#25D366] opacity-60 animate-ping"></span>
        <svg className="relative w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
      </a>
